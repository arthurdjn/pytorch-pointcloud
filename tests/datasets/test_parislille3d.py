...
def test_parislille3d_dataset_download_unsupported(datasets_dir_factory: Callable[..., Path]) -> None:
    """`download()` raises because Paris-Lille-3D must be downloaded manually."""
    datasets_dir = datasets_dir_factory("ParisLille3D/raw/**/*")
    dataset = ParisLille3D(root=datasets_dir, split="all")

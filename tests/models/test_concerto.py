...
@pytest.fixture
def model() -> ConcertoSegmentation:
    return ConcertoSegmentation(
        in_channels=6,
        num_classes=10,
...
def test_concerto_segmentation_forward(model: ConcertoSegmentation, data: Dict[str, Tensor]) -> None:
    logits = model(data["x"], data["pos_grid"], data["batch"])
    assert logits.shape == (data["pos_grid"].shape[0], model.num_classes)


def test_concerto_segmentation_reset_classifier(model: ConcertoSegmentation, data: Dict[str, Tensor]) -> None:
...
def test_concerto_segmentation_default_norm_no_batch_leak() -> None:
    torch.manual_seed(0)
    ...
    with torch.no_grad():
        batched = model(torch.cat(xs).cuda(), torch.cat(pos_grids).cuda(), batch.cuda())
        separate = model(xs[0].cuda(), pos_grids[0].cuda(), torch.zeros(lengths[0], dtype=torch.long).cuda())

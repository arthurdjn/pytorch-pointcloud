...
from torch_pointcloud.datasets import ModelNet40Hdf5, ScanObjectNN
...
def build_dataset(args: argparse.Namespace, transform: Any) -> Union[ModelNet40Hdf5, ScanObjectNN]:
    spec = MODELS[args.model]
    if spec["dataset"] == "modelnet40":
        return ModelNet40Hdf5(
            root=args.root,
            train=False,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark the PointMLP classifiers on ModelNet40 and ScanObjectNN.")
    parser.add_argument("--model", default="pointmlp-base.modelnet40.xu-ma", choices=sorted(MODELS))

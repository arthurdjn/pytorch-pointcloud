"""Benchmark the DGCNN ShapeNetPart part-segmentation model (single pass, no voting).

Results (instance mIoU / class mIoU):

    | Variant                   | reference | torch-pointcloud |
    | ------------------------- | --------- | ---------------- |
    | dgcnn.shapenetpart.an-tao | 85.2      | 85.23 / 80.92    |

Usage:
    uv run --no-sync python examples/dgcnn_benchmark_part_segmentation.py
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark DGCNN part segmentation on ShapeNetPart.")
    parser.add_argument("--model", default="dgcnn.shapenetpart.an-tao", help="Registered segmentation model name")

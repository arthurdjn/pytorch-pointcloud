...
fully sparse multi-group head predicts boxes directly on voxels (no dense BEV map). The dataset
aggregates `max_sweeps` LiDAR sweeps per keyframe and converts the global-frame annotations to LiDAR
boxes; results are scored with the generic oriented-3D mAP (not the official nuScenes NDS). Defaults to
the `v1.0-mini` split (404 keyframes).

Results vs reference:

    | Source            | nuScenes val                                                          |
    | ----------------- | -------------------------------------------------------------------- |
    | VoxelNeXt (paper) | NDS 66.6, mAP 60.5                                                    |
    | torch-pointcloud  | mAP@0.25 58.24, @0.5 38.55 (v1.0-mini smoke, not NDS; bit-exact ~1e-6) |

...
def main() -> None:
    ...
    print("Evaluating...")
    metrics = evaluate(model, test_dataloader, args.device, num_classes=num_classes, with_normals=args.with_normals)
...
def parse_args() -> Namespace:
    parser = ArgumentParser(description="Benchmark PointNet++ classification on ModelNet40 (normal-resampled).")
    ...
    args.with_normals = "msg" in args.model
    return args
...
def evaluate(
    ...
    *,
    ...
    for data in pbar:
        ...
        x = data[DataKeys.NORMAL].to(device) if with_normals else None

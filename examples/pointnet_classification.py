...
from torch_pointcloud.models import PointNetClassification
...
def main() -> None:
    ...
    model = PointNetClassification(num_classes=10, features_dim=3).to(args.device)
...
def train_one_epoch(
    ...
    for i, data in pbar:
        ...
        features = data["normal"].to(device)
        batch = data["batch"].to(device)

        optimizer.zero_grad()
        logits = model(coords, features, batch)
    ...
    return {"train/loss_epoch": cum_loss / len(dataloader)}
...
def eval_one_epoch(model: Module, dataloader: DataLoader, device: str = "cuda") -> Dict[str, float]:
    ...
    for data in tqdm(dataloader, total=len(dataloader), desc="Evaluating"):
        ...
        with torch.no_grad():
            preds = model(coords, features, batch).max(1)[1]

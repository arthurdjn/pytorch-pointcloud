...
Usage:
    ...
    uv run --no-sync python examples/spunet_benchmark_segmentation.py --download
"""
...
def evaluate(model: Module, dataloader: DataLoader, inferer: Inferer, device: str, num_classes: int) -> Dict[str, Any]:
    ...
    for data in pbar:
        ...
        probs = inferer(data, predictor=lambda d: model(d[DataKeys.X], d[DataKeys.POS_GRID], d[DataKeys.BATCH]))

...
Results vs reference (instance mIoU / class mIoU):
...
def main() -> None:
    ...
    transform = T.Compose(
        [
            ...
            T.RandomSample(keys=("pos", "segment"), num_samples=NUM_POINTS, replace=True),
        ]
    )

    print("Loading ShapeNetPart test dataset!")
    test_dataset = ShapeNetPart(root=args.root, split="test", transform=transform)
...
def parse_args() -> Namespace:
    parser = ArgumentParser(description="Benchmark Point-MAE part segmentation on ShapeNet.")

"""Benchmark SphereFormer semantic segmentation on SemanticKITTI (single pass, full-resolution scoring).

NOTE: the released weights are no longer downloadable (dvlab issue #78); this script evaluates a local checkpoint of the
ported architecture, which matches the reference build to 2.4e-7.

Results (val sequence 08, mIoU):

    | Variant                    | reference | torch-pointcloud |
    | -------------------------- | --------- | ---------------- |
    | sphereformer.semantickitti | 67.8      |                  |

Usage:
    uv run --no-sync python examples/sphereformer_benchmark_segmentation.py --checkpoint sphereformer.pt --limit 5
"""
...
def evaluate(model: Module, dataloader: DataLoader, inferer: Inferer, device: str, num_classes: int) -> Dict[str, Any]:
    ...
    for data in pbar:
        ...
        scores = inferer(
            data, predictor=lambda d: model(d[DataKeys.X], d[DataKeys.POS], d[DataKeys.POS_GRID], d[DataKeys.BATCH])
        )
...
def main() -> None:
    ...
    model, model_info = create_model(
        args.model, task="semantic-segmentation", checkpoint_path=args.checkpoint, return_info=True
    )

...
class LocalSpatialEncoding(nn.Module):
    ...
    def forward(self, coords, features, knn_output):
        ...
        concat = torch.cat((extended_coords, neighbors, extended_coords - neighbors, dist.unsqueeze(-3)), dim=-3).to(
            self.device
        )
...
class AttentivePooling(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.score_fn = nn.Sequential(nn.Linear(in_channels, in_channels, bias=False), nn.Softmax(dim=-2))
...
class RandLANet(nn.Module):
    def __init__(self, d_in, num_classes, num_neighbors=16, decimation=4, device=torch.device("cpu")):
        ...
        self.bn_start = nn.Sequential(nn.BatchNorm2d(8, eps=1e-6, momentum=0.99), nn.LeakyReLU(0.2))

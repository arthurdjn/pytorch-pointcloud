...
from .pointpillars import _OPENPCDET_NORM_KWARGS
    ...
    def __init__(
        ...
        *,
        ...
        self.conv1 = spconv.SparseSequential(
            _post_act_block(16, 16, 3, indice_key="subm1", **block_kwargs),
        )
        self.conv2 = spconv.SparseSequential(
            _post_act_block(16, 32, 3, stride=2, padding=1, indice_key="spconv2", conv_type="spconv", **block_kwargs),
            _post_act_block(32, 32, 3, indice_key="subm2", **block_kwargs),
            _post_act_block(32, 32, 3, indice_key="subm2", **block_kwargs),
        )
        self.conv3 = spconv.SparseSequential(
            _post_act_block(32, 64, 3, stride=2, padding=1, indice_key="spconv3", conv_type="spconv", **block_kwargs),
            _post_act_block(64, 64, 3, indice_key="subm3", **block_kwargs),
            _post_act_block(64, 64, 3, indice_key="subm3", **block_kwargs),
        )
        self.conv4 = spconv.SparseSequential(
            _post_act_block(
                64, 64, 3, stride=2, padding=(0, 1, 1), indice_key="spconv4", conv_type="spconv", **block_kwargs
            ),
            _post_act_block(64, 64, 3, indice_key="subm4", **block_kwargs),
            _post_act_block(64, 64, 3, indice_key="subm4", **block_kwargs),
        )
...
class SECOND(DetectionModel):
    ...
    Args:
        ...
        max_num_points: Maximum points kept per voxel.
        max_num_voxels: Maximum voxels kept per scene.
    ...
    def forward_features(self, x: OptTensor, pos: Tensor, batch: Tensor) -> Tensor:
        ...
        normalizer = torch.clamp_min(num_points.view(-1, 1), min=1.0).type_as(voxels)
...
class SECONDMultiHead(DetectionModel):
    ...
    def forward_features(self, x: OptTensor, pos: Tensor, batch: Tensor) -> Tensor:
        ...
        sparse_tensor = spconv.SparseConvTensor(
            features=voxel_features, indices=coords.int(), spatial_shape=self.sparse_shape, batch_size=batch_size
        )
        encoded = self.backbone_3d(sparse_tensor)
        dense = encoded.dense()
        b, c, d, h, w = dense.shape

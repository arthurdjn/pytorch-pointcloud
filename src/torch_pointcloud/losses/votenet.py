...
$(B, N, \cdot)$. The seed and vote tensors are expected dense as $(B, S, \cdot)$ with $S$ the
number of seeds; the training module reshapes the model's packed seeds to this layout and
supplies `seed_inds`, the index of each seed into the original $N$ points, so the per-point
vote labels can be gathered onto seeds.
...
class VoteNetLoss(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.register_buffer("mean_size_arr", mean)

    def forward(self, output: Dict[str, Tensor], batch: Dict[str, Any]) -> Dict[str, Tensor]:
        r"""Compute the VoteNet loss and its components.

        Args:
            output: Detection head outputs (`objectness_scores`, `center`, `heading_scores`,
                `heading_residuals_normalized`, `size_scores`, `size_residuals_normalized`,
                `sem_cls_scores`, `aggregated_vote_pos` as $(B, K, \cdot)$, plus dense
                `seed_pos`, `vote_pos` $(B, S, 3)$ and `seed_inds` $(B, S)$).
            batch: Ground truth (`center_label`, `heading_class_label`, `heading_residual_label`,
                ...
                $(B, M, \cdot)$, plus per-point `vote_label` $(B, N, 9)$ and `vote_label_mask` $(B, N)$).
    ...
    def _vote_loss(self, output: Dict[str, Tensor], batch: Dict[str, Any]) -> Tensor:
        ...
        seed_pos = output["seed_pos"]
        vote_pos = output["vote_pos"]
        seed_inds = output["seed_inds"].long()
        vote_label: Tensor = batch["vote_label"]
        vote_label_mask: Tensor = batch["vote_label_mask"]

        batch_size, num_seed = seed_pos.shape[:2]
        num_candidates = vote_label.size(-1) // 3
        vote_factor = vote_pos.size(1) // num_seed

        seed_gt_votes_mask = torch.gather(vote_label_mask, 1, seed_inds).float()
        gather_idx = seed_inds.unsqueeze(-1).expand(-1, -1, vote_label.size(-1))
        seed_gt_votes = torch.gather(vote_label, 1, gather_idx) + seed_pos.repeat(1, 1, num_candidates)

        pred = vote_pos.reshape(batch_size * num_seed, vote_factor, 3)
    ...
    def _objectness_loss(
        ...
        dist1, idx1, _, _ = _nn_distance(output["aggregated_vote_pos"], gt_center)
    ...
    def _box_and_sem_loss(
        ...
        mean_size = (size_one_hot * self.mean_size_arr.view(1, 1, ns, 3)).sum(dim=2)
